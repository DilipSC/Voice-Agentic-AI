...
def to_pgvector(emb_list):
    """
    pgvector accepts '[0.1,0.2]' literal format.
    No ::vector type cast needed.
    """
...
def save_message_with_embedding(
    ...
    with engine_.begin() as conn:
        ...
        # IMPORTANT FIX: no ::vector and only SQLAlchemy params
        conn.execute(
            text("""
...
# -----------------------------------------------------------
# LONG-TERM SUMMARY MEMORY (compressed conversation)
# -----------------------------------------------------------
...
- Context that helps future responses
- Important facts or ongoing tasks
...
# -----------------------------------------------------------
# LLM RESPONSE (uses: summary + semantic + recent)
# -----------------------------------------------------------
def generate_reply(
    recent_msgs,
    user_text: str,
    ...
    long_term_summary = summary or "No summary exists yet."
    ...
    # Recent chat block
    recent_block = "".join(
        f"{(m.role if hasattr(m, 'role') else m['role']).upper()}: "
        f"{(m.content if hasattr(m, 'content') else m['content'])}\n"
        for m in recent_msgs
    )

    prompt = f"""
You are a realtime voice AI assistant. Maintain memory, be concise, and sound natural.
...
Respond as a conversational assistant. Keep it short and clear.
"""
